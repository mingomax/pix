...
describe('Integration | Repository | AuthenticationMethod', () => {
  ...
  describe('#findOneByUserIdAndIdentityProvider', () => {
    ...
    it('should return the AuthenticationMethods associated to a user for a given identity provider', async () => {
      // given
      const identityProvider = AuthenticationMethod.identityProviders.GAR;
      const userId = databaseBuilder.factory.buildUser().id;
    ...
    it('should return null if there is no AuthenticationMethods for the given user and identity provider', async () => {
      // given
      const userId = databaseBuilder.factory.buildUser().id;
      await databaseBuilder.commit();
      const identityProvider = AuthenticationMethod.identityProviders.GAR;
  ...
  describe('#updateExternalIdentifierByUserIdAndIdentityProvider', () => {
    ...
    context('When authentication method exists', async () => {
      ...
      beforeEach(() => {
        const userId = databaseBuilder.factory.buildUser().id;
        authenticationMethod = databaseBuilder.factory.buildAuthenticationMethod({ identityProvider: AuthenticationMethod.identityProviders.GAR, userId });
    ...
    context('When authentication method does not exist', async () => {
      ...
      it('should update sub by userId and type', async () => {
        // given
        const userId = 12345;
        const identityProvider = AuthenticationMethod.identityProviders.GAR;
        const externalIdentifier = 'new_saml_id';
